...
# These function were moved from hook-_tkinter.py

import locale
import os

from PyInstaller.building.datastruct import Tree
from PyInstaller.compat import (is_win, is_venv, base_prefix, open_file,
                                text_read_mode, modname_tkinter, is_darwin)
...
def handle_broken_tcl_tk():
    ...
    environments. Older versions of `virtualenv` set such variables
    erroneously, preventing PyInstaller from properly detecting Tcl/Tk. This
    issue has been noted for `virtualenv` under Python 2.4 and Windows 7.
...
def warn_if_activetcl_or_teapot_installed(tcl_root, tcltree):
    """
    If the current Tcl installation is a Teapot-distributed version of
    ActiveTcl *and* the current platform is OS X, log a non-fatal warning
    ...
    # <https://www.tcl.tk/doc/howto/i18n.html#system_encoding>`_.
...
def find_tcl_tk_darwin_frameworks(binaries):
    """
...
def collect_tcl_tk_files(file):
    ...
    handle_broken_tcl_tk()

    tcl_root, tk_root = find_tcl_tk(file)
